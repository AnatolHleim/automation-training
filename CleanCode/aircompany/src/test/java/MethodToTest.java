...
public class MethodToTest {
  ...
  static PassengerPlane planeWithMaxPassengerCapacity = new PassengerPlane(new Plane("Boeing-747", 980, 16100, 70500), 242);

  public boolean getTransportMilitaryPlanes() {
    Airport airport = new Airport(planes);
    List<MilitaryPlane> transportMilitaryPlanes = airport.getTransportMilitaryPlanes();
    boolean militaryPlaneisTransport = false;
    for (MilitaryPlane militaryPlane : transportMilitaryPlanes) {
      if ((militaryPlane.getType() == MilitaryType.TRANSPORT)) {
        militaryPlaneisTransport = true;
        break;
      }
    }
    return militaryPlaneisTransport;
  }

  public static boolean nextPlaneMaxLoadCapacityIsHigherThanCurrent() {
    Airport airport = new Airport(planes);
